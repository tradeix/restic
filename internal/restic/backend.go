...
type Backend interface {
	...
	Test(ctx context.Context, h  file.Handle) (bool, error)

	// Remove removes a File described  by h.
	Remove(ctx context.Context, h  file.Handle) error

	// Close the backend
	Close() error

	// Save stores the data from rd under the given handle.
	Save(ctx context.Context, h  file.Handle, rd RewindReader) error
	...
	Stat(ctx context.Context, h  file.Handle) (FileInfo, error)
	...
	// from.
	List(ctx context.Context, t file.FileType, fn func(FileInfo) error) error
